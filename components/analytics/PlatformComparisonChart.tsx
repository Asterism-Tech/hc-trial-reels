...
export default function PlatformComparisonChart({ groups }: Props) {
  ...
  if (platforms.length === 0) return (
    <div className="flex items-center justify-center h-32 text-[#555] text-sm">No platform data yet</div>
  )
        ...
        >
          ...
          <div className="space-y-2">
            <div className="flex justify-between">
              <span className="text-xs text-[#666]">Avg Views</span>
              <span className="text-xs font-mono text-white">{formatNumber(avg(s.views))}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-xs text-[#666]">Avg Completion</span>
              <span className="text-xs font-mono text-white">{avgPct(s.completion)}%</span>
            </div>
            <div className="flex justify-between">
              <span className="text-xs text-[#666]">Avg Saves</span>
              <span className="text-xs font-mono text-white">{formatNumber(avg(s.saves))}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-xs text-[#666]">Versions tracked</span>
              <span className="text-xs font-mono text-[#888]">{s.views.length}</span>
            </div>
          </div>
        </div>
      ))}
